package com.splunk.android.instrumentation.recording.wireframe.descriptor.internal

import com.splunk.android.common.utils.extensions.toClass
import com.splunk.android.instrumentation.recording.wireframe.descriptor.RelativeLayoutDescriptor

internal open class TextInputTimePickerViewDescriptor : RelativeLayoutDescriptor() {

    override val intendedClass: Class<*>? = "android.widget.TextInputTimePickerView".toClass()
}
