...
package com.splunk.android.sr.testapp.ui.slowrendering
...
class SlowRenderingFragment : BaseFragment<FragmentSlowRenderingBinding>() {
    ...
    private fun simulateSlowRendering(
        ...
        val slowRenderView = SlowRenderView(requireContext(), renderDelayMs, color, refreshIntervalMs)
        slowRenderView.layoutParams = ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT)

        val rootView = requireActivity().contentView ?: throw NullPointerException()
        rootView.addView(slowRenderView)

        Toast.makeText(requireContext(), "Running $testName test for ${durationMs / 1_000} seconds", Toast.LENGTH_SHORT).show()

        Handler(Looper.getMainLooper()).postDelayed(durationMs) {
            slowRenderView.stopRendering()
            rootView.removeView(slowRenderView)

            Toast.makeText(requireContext(), "$testName test completed", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private class SlowRenderView(
        ...
        private fun startSlowRendering() {
            invalidate()
            renderHandler.postDelayed(refreshIntervalMs) { startSlowRendering() }
        }
            ...
            while (SystemClock.elapsedRealtime() - renderStartTime < renderDelayMs)
                if (width > 0 && height > 0)
                    repeat(CIRCLES_PER_FRAME) {
                        val x = randomGenerator.nextFloat() * width
                        val y = randomGenerator.nextFloat() * height

                        canvas.drawCircle(x, y, CIRCLE_RADIUS, shapePaint)
                    }
        }
