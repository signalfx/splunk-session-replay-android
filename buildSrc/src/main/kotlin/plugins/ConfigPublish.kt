...
import utils.addCiscoInfo
import utils.artifactIdProperty
import utils.defaultGroupId
import utils.isReleasedProperty
import utils.propOrDefault
import utils.toBoxString
...
class ConfigPublish : Plugin<Project> by local plugin {
    ...
    afterEvaluate {
        ...
        if (project.propOrDefault(isReleasedProperty, true) as Boolean) {
            tasks.register("publishMavenPublicationToMavenLocalIfReleased") {
                group = TaskGroups.PUBLISHING
                setFinalizedBy(listOf("publishMavenPublicationToMavenLocal"))
            }
            tasks.register("artifactId") {
                group = TaskGroups.PUBLISHING
                doLast {
                    println("${project.properties[artifactIdProperty]}")
                }
            }
            tasks.register("publishIfReleased") {
                group = TaskGroups.PUBLISHING
                setFinalizedBy(listOf("publish"))
            }
        }
    }
    ...
    tasks.withType(AbstractPublishToMaven::class.java) {
        doLast {
            ...
            val publication = project.extensions.getByType<PublishingExtension>().publications.getByName("maven") as MavenPublication
